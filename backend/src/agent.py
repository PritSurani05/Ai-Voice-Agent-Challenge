...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession[Userdata](
        ...
        # Optimized for low latency: word tokenizer for faster streaming, no text pacing
        tts=murf.TTS(
                voice="en-US-matthew", 
                style="Conversation",
                tokenizer=tokenize.basic.WordTokenizer(),  # Word-level for faster streaming
                text_pacing=False  # Disable text pacing to reduce latency
            ),
        # VAD and turn detection are used to determine when the user is speaking and when the agent should respond
        # See more at https://docs.livekit.io/agents/build/turns
        # Optimized for faster turn detection
    ...
    await ctx.connect()
    logger.info("Agent connected to room, session is active and listening")
    ...
    # Check if there are any participants in the room
    participants = list(ctx.room.remote_participants.values())
    logger.info(f"Remote participants in room: {len(participants)}")
    for p in participants:
        logger.info(f"  - {p.identity} (name: {p.name})")
    
    # The agent will automatically start listening for user speech
    # The instructions tell the agent to greet immediately when conversation starts
    # We don't need to trigger manually - the agent will respond when it detects speech
    logger.info("Agent is ready and waiting for user speech...")
    
    # Trigger a proactive greeting after a short delay if user is connected
    async def proactive_greeting():
        await asyncio.sleep(0.5)  # Reduced wait time for faster response
        participants = list(ctx.room.remote_participants.values())
        if len(participants) > 0:
            logger.info("User detected, agent should greet proactively")
            # The agent will automatically greet when it detects the first user speech
            # No additional delay needed
    
    # Start proactive greeting task
