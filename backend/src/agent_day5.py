"""Day 5 Razorpay SDR Agent – FAQ-driven lead capture."""
...
class RazorpayKnowledgeBase:
    ...
    @classmethod
    def from_disk(cls) -> "RazorpayKnowledgeBase":
        env_path = os.getenv("DAY5_FAQ_PATH")
        default_path = Path(__file__).resolve().parents[2] / "shared-data" / "day5_razorpay_faq.json"
    ...
    def answer(self, query: str) -> str:
        ...
        if not matches:
            return (
                "I could not find that in the Razorpay FAQ set I'm loaded with. "
    ...
    def pricing_summary(self) -> str:
        if not self.pricing:
            return "Standard Razorpay pricing starts at 2% domestic / 3% international per transaction."
        base = self.pricing
        return (
            f"Standard plan: {base.get('domestic')} International: {base.get('international')} "
            f"Setup fee: {base.get('setup_fee')} / Annual maintenance: {base.get('maintenance')}. "
            f"Hidden charges: {base.get('hidden_charges')} Enterprise: {base.get('enterprise')}"
        )
...
class Userdata:
    lead: LeadCapture
    knowledge: RazorpayKnowledgeBase
    storage: LeadStorage
    lead_id: str
...
class RazorpaySdrAgent(Agent):
    """Live SDR persona that leans on Razorpay FAQ content."""

    def __init__(self, *, userdata: Userdata) -> None:
        instructions = f"""You are a warm, proactive Sales Development Representative for Razorpay, India's leading full-stack payments company.
Key persona guidelines:
- Greet every caller with energy, mention Razorpay briefly, and ask what brought them in plus what they are building.
...
- After you capture name or company details, acknowledge them and explain how Razorpay can help based on their use case.
...
- Keep replies crisp, professional, and friendly—think modern fintech SDR."""
    ...
    async def answer_company_question(self, ctx: RunContext[Userdata], question: str) -> str:
        """Grounded answer for any Razorpay company/product/pricing/document question."""
    ...
    async def summarize_pricing_basics(self, ctx: RunContext[Userdata]) -> str:
        """Quick reminder of Razorpay's published pricing structure."""
        return ctx.userdata.knowledge.pricing_summary()

    @function_tool
    async def list_activation_documents(self, ctx: RunContext[Userdata]) -> str:
        """List the KYC documents required to activate a live Razorpay account."""
    ...
    async def finalize_lead(self, ctx: RunContext[Userdata]) -> str:
        ...
        return summary_sentence + " Thanks for the chat—watch your inbox for a Razorpay follow-up."


def prewarm(proc: JobProcess, silero_module):
    proc.userdata["vad"] = silero_module.VAD.load()
    proc.userdata["razorpay_kb"] = RazorpayKnowledgeBase.from_disk()
...
async def entrypoint(ctx: JobContext):
    ...
    knowledge = ctx.proc.userdata.get("razorpay_kb", RazorpayKnowledgeBase.from_disk())
    ...
    agent = RazorpaySdrAgent(userdata=userdata)
    ...
    await ctx.connect()
    logger.info("Day 5 Razorpay SDR agent connected and listening.")
